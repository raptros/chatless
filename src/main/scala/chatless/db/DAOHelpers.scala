...
trait DAOHelpers { this: SalatDAO[_, _] =>

  type ID
  implicit val idAsQueryParam: AsQueryParam[ID]

  def setOneField[A: Manifest : AsQueryParam](id: ID, field: String, newVal: A): WriteStat = {
    val modifyResult: mongodb.WriteResult = collection.update(
  ...
  def addToSet[A : Manifest : AsQueryParam](id: ID, field: String, newItem: A): WriteStat = {
    val modifyResult = collection.update(
